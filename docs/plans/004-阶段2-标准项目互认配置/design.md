...
同一组织编码和标准项目编码只能有一条配置，停用配置仍占用唯一键；新增写入前按组织编码与标准项目编码查重，命中即按重复配置拒绝，并发下两个请求可能同时通过查重，此时由唯一键拒绝。创建后默认启用；普通修改只修改可互认时间；启用和停用为独立操作，重复同方向操作幂等成功且不登记事件。目录停用时保留配置，允许修改时间和停用配置；重新启用要求标准项目、分类和分组当前有效。
