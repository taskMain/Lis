...
阶段设计见 [Server/design.md](Server/design.md) 与 [Client/design.md](Client/design.md)；批次与矩阵编号见 [impl.md](impl.md)、[Server/impl.md](Server/impl.md) 和 [Client/impl.md](Client/impl.md)；测试结果见 [testReport.md](testReport.md)；设计讨论记录见 [阶段3-讨论内容.md](阶段3-讨论内容.md)。
...
| S3-D11 | 医院管理员入口在可信上下文缺少组织或医院时拒绝，不降级、不使用默认值 | DesignConfirmed | `CONTEXT.md`「组织、医院和院区」；SRS F10 前置条件 2 | Application | V14 |
| S3-D12 | 首次并发保存同一业务键时以唯一索引兜底，冲突翻译为业务拒绝（提示刷新后重试），不自动重试 | DesignConfirmed | SRS F10 未定义并发语义；`backend-command-query-event` 第 4.1 节要求说明并发兜底；沿用"不替用户循环重放意图"的既有口径 | Repository、Application | V16 |
