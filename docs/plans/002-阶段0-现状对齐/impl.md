...
1. 监听地址三处一致，均为 `http://localhost:15014`（`appsettings.json`、`appsettings.Development.json`、`Properties/launchSettings.json`）。`launchSettings.json` 只有 1 个 profile，`launchUrl` 为 `scalar`。
