...
8. **不得把对话内容写进仓库。** 决策的依据列只引 SRS、UML、项目规范或既有先例；引用不到客观上游时写技术理由。禁止出现"负责人批注""裁定原文""确认来源""回复原文"等表述，也不得为留档对话而新建文件。
...
| 长期业务规则、领域事实和公共业务契约 | 当前 SRS、UML；入口见 Project Context |
...
冲突依次遵循：用户本轮明确要求、`AGENTS.md`、本轮适用规范、总体计划、阶段设计、实施与测试矩阵。业务事实以已确认决策及本项目 SRS/UML 为准；迁入的通用规范、角色提示、技能和示例不能暗中覆盖业务契约。发现未确认冲突时，只暂停受影响范围并请求负责人决策。
