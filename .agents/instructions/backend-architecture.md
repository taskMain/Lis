...
- 按结果行逐条调用外部服务或数据库往返（N+1）；附属数据必须批量读取后在内存映射填充，见第 5.3 节。
...
查询实现不得按结果行逐条发起外部服务调用或数据库往返。一个查询的调用次数只能随**筛选维度**（例如组织、医院、院区的数量）增长，不得随**返回行数**增长。

- 名称、编码对照、组织路径等附属数据必须一次性批量取回，在内存中建立映射后逐行填充；不得在遍历结果行时对每一行重复读取同一份附属数据。
- 关联本系统数据的展示字段优先在同一条 SQL 内取得，不得拆成"先查主体、再逐行补明细"。
- 重复的读取目标先去重再读取，同一响应内同一个业务键只读取一次。
- 外部契约缺少批量方法时，先评估"一次取全集 + 内存映射"是否可行；不可行再回到设计确认，不得默认退化为逐行调用。
- 该约束适用于所有查询入口，包括本系统查询接口，以及通过外部系统接口读取数据的查询。
- 验证方式：查询类测试必须能证明调用次数与返回行数无关（例如用计数替身断言对外调用次数），不能只依据代码审阅。
