...
| 后端启动命令与监听地址 | 已实际验证：监听 `http://localhost:5008`，OpenAPI `/openapi/v1.json` 与 Scalar `/scalar` 均可访问。启动配置三处一致（`appsettings.json`、`appsettings.Development.json`、`Properties/launchSettings.json`）。业务接口当前不可用，原因见 §3.4 |
...
| 步骤 | 命令或事实 |
|---|---|
| 取原始文档 | `GET http://localhost:5008/openapi/v1.json`（需后端已启动） |
...
用户于 2026-09-11 澄清本平台尚未建立数据库表：19 张平台表均待按所属阶段独立新建，物理表名为 `mrec_` 加已选业务基础表名，不强制与实体名完全一致。阶段 1 保留现有脚本基础名，仅新建 `mrec_medical_standard_category`、`mrec_medical_standard_group`、`mrec_medical_standard_item`，其余 16 张随所属业务阶段处理。此为用户确认的基线，不是本轮数据库查询结果。
