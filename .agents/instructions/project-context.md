...
本项目已确认的业务科室/人员规则与通用平台身份规则不同：医院业务请求中的科室和人员 ID 是字符串，名称随请求保存，不要求存在于平台权限系统，不按 ID 补查或覆盖业务名称。组织、医院和院区仍由可信上下文确定并校验。权限系统负责菜单和接口授权，本项目不重复建设角色授权模型，但仍保护数据归属范围。具体字段、必填和统计规则以 SRS/UML 为准。
