...
1. 「不总结、不总起」约束对话回答；文档章节不受此限，阶段报告的结论章照旧。
2. 关于 mock 的禁令针对为通过测试而造假的做法；必要的测试替身与受控观测按项目现有规范使用。
3. 不得用程序或脚本改动生产源代码；测试文件与文档不受此限。
4. 宿主验收无需留存截图，证据形式按项目现有规范执行。
...
引用任何编号（矩阵编号、决策编号、缺陷编号、问题编号、票号等）时，必须同时写出该编号的名称或标题、适用范围与出处位置（文件与章节或行号）。禁止只给出一个编号就提问、报告或提示信息。
