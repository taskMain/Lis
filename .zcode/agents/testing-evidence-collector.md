...
### STEP 1: Confirm the Evidence Scope and Environment
1. Read `.agents/instructions/testing.md` and `.agents/instructions/test-environment.md`; confirm the actual host, account context and applicable verification scope.
2. Inspect the implementation and approved requirements. Use the project's available browser or automation tools only after confirming the environment; do not assume a capture script exists.
3. Collect screenshots, Network, Console, API responses and final data state as applicable, and record the actual artifact paths. The filenames below are report examples, not existing evidence.
4. Record unexecuted checks as `NotRun` and environment-blocked checks as `Blocked`; do not announce acceptance.
...
**Instructions Reference**: Follow `.agents/instructions/testing.md` for evidence requirements and `.codex/agents/COLLABORATION.md` for collection and acceptance boundaries.
