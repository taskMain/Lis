...
/// 普通检验结果的只读返回数据。
/// </summary>
public sealed record LaboratoryResultItemReadModel
{
  ...
  /// <summary>
  /// 结果类型中文；由服务端按枚举声明解析，页面直接展示。
  /// </summary>
  public string ResultTypeText => EnumDescriptorText.Get(ResultType, LaboratoryResultTypeDescriptorList.List);
  ...
  /// <summary>异常标志；来源未提供时为 <see langword="null"/>。</summary>
  public LaboratoryAbnormalFlag? AbnormalFlag { get; init; }
  /// <summary>
  /// 异常标志中文；来源未提供时为 <see langword="null"/>。
  /// </summary>
  public string? AbnormalFlagText => AbnormalFlag is LaboratoryAbnormalFlag flag
    ? EnumDescriptorText.GetOrNull(flag, LaboratoryAbnormalFlagDescriptorList.List)
    : null;
