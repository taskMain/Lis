...
/// <summary>
/// 接收侧互认使用汇总查询：平台管理员入口，接收组与来源组两组范围条件都取自请求。
/// </summary>
/// <remarks>
/// 范围条件均可选，请求级空取值不附加该层过滤（全空即平台按授权全量）；
/// 提供的取值由服务端经组织路径解析校验存在、启用与父子归属；两组组织编码同时提供且不一致时按业务拒绝。
/// 汇总维度必填，且取值必须是枚举已定义值；日期范围为必填的本地日闭区间。
/// </remarks>
public sealed record RecognitionUsageSummaryQueryRequest
{
