...
public partial class MedicalRecognitionReportManager : DomainService
{
  ...
  /// </summary>
  /// <remarks>
  /// 标准项目、所属分类与所属分组必须当前有效；配置以启用状态创建，同一组织与同一标准项目只能有一条配置，停用配置仍占用该配置。
  /// 写入前按组织编码与标准项目编码查重，命中即按重复配置拒绝；并发下两个请求可能同时通过查重，
  /// 此时由"组织编码 + 标准项目编码"唯一索引拒绝，数据库异常按原样向外传播。
  /// </remarks>
  /// <param name="command">新增配置命令，携带可信组织、应用层按编码解析出的标准项目标识与操作字段。</param>
  /// <returns>创建成功返回 <see langword="true"/>。</returns>
  /// <exception cref="InvalidOperationException">
  /// 标准项目、所属分类或所属分组不存在或已停用，该组织已配置此标准项目（含已停用配置占用的配置），或配置影响的行数不为 1（含 0 行）。
  /// </exception>
  public async Task<bool> CreateMutualRecognitionItemAsync(CreateMutualRecognitionItemCommand command)
  {
    ...
    // 停用配置同样占用"组织编码 + 标准项目编码"唯一键，因此查重不筛选启用状态。
    if (await repository.GetMutualRecognitionItemByOrganizationAndProjectAsync(command.OrganizationCode, command.StandardProjectCode) is not null)
      throw new InvalidOperationException("业务拒绝：该组织已配置此标准项目。");
  ...
  /// 写入前按四个业务键查重：无记录则新增，有记录则按主键与四个业务键列条件更新，因此普通重复保存不会命中唯一索引；
  /// 并发下两个请求可能同时通过查重，此时由四列唯一索引拒绝，数据库异常按原样向外传播、不登记事件、不自动改用更新路径。
  /// 本方法只执行一条写语句，因此不控制事务。
  /// </remarks>
  ...
  /// 条件更新影响 0 行且复读无法解释为成功，或影响行数大于 1 时抛出。
  /// </exception>
