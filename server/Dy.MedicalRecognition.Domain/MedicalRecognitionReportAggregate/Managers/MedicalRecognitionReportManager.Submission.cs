...
public partial class MedicalRecognitionReportManager
{
  ...
  /// 报告已作废、患者身份冲突或报告跨患者迁移时抛出；此时不保存任何内容、不登记事件。
  /// </exception>
  ...
  /// <exception cref="InvalidOperationException">报告已作废时抛出。</exception>
  ...
  /// 首次解析按证件键读取，未命中才创建，因此普通重复提交不会命中唯一索引；并发下两个请求可能同时通过读取，
  /// 此时由证件键唯一索引拒绝，数据库异常按原样向外传播。
  ...
  /// <exception cref="InvalidOperationException">跨患者迁移或核心身份冲突时抛出。</exception>
