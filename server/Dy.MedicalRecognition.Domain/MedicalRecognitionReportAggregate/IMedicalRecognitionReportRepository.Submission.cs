...
/// </summary>
/// <remarks>
/// 只做数据读写并返回行数或查询结果，不判断业务状态；唯一约束冲突由实现识别并按持久化事实翻译为内部异常类型，
/// 是否构成业务拒绝仍由领域管理器决定，本层不改变对外响应形态。
/// </remarks>
public partial interface IMedicalRecognitionReportRepository
{
  ...
  /// <summary>
  /// 并发首插撞证件键后复读平台患者：读得到时返回患者，读不到时区分「不存在」与「当前事务已不可读」。
  /// </summary>
  /// <remarks>
  /// 并发插入同一证件键时，唯一约束冲突会把所在事务标记为终止，此后该事务内的任何语句都会被数据库拒绝；
  /// 领域层因此无法用普通读取区分「没有这个患者」与「读不出来」。本方法把这两种情形分开返回，
  /// 由领域层决定按患者不存在继续、还是按并发冲突拒绝，不在领域层引入具体数据库 Provider 的错误码。
  /// </remarks>
  /// <param name="identityDocumentTypeCode">证件类型代码。</param>
  /// <param name="identityDocumentNo">证件号码。</param>
  /// <returns>患者实体与本次读取是否可用；读取因事务终止不可用时 <c>IsReadable</c> 为 <see langword="false"/>。</returns>
  Task<(PlatformPatient? Patient, bool IsReadable)> TryGetPlatformPatientByDocumentAsync(string identityDocumentTypeCode, string identityDocumentNo);
  /// <summary>
  /// 插入一个平台患者。
  /// </summary>
  /// <remarks>并发首次解析同一证件时由证件类型代码与证件号码唯一约束拒绝。</remarks>
  /// <param name="platformPatient">待插入的患者实体。</param>
  /// <returns>受影响行数。</returns>
  /// <exception cref="DuplicatePlatformPatientException">写入违反证件键唯一约束时抛出。</exception>
  ...
  /// <returns>受影响行数。</returns>
  /// <exception cref="DuplicateMedicalReportVersionException">写入违反报告与版本序号唯一约束时抛出。</exception>
